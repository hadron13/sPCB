...
void ti7404_update(component_t *component, double dt){
    ...
    //     output_pin->mode        = a? DRAINPUT  :SOURCE;
...
component_t ti7400_init(){
    ...
    // pin_t *pins = component.pins;
    //
    // pins[0].type = INPUT;     pins[13].type = SUPPLY;
    // pins[1].type = INPUT;     pins[12].type = IN; 
    // pins[2].type = OUT;    pins[11].type = INPUT; 
    // pins[3].type = INPUT;     pins[10].type = OUT; 
    // pins[4].type = INPUT;     pins[9].type  = IN; 
    // pins[5].type = OUT;    pins[8].type  = INPUT;  
...
void ti7400_update(component_t *component, double dt){
    ...
    //
    //     output_pin->mode        = (a && b)?DRAINPUT:SOURCE;
